             ...
             const bool PbP = false) // Stores if this run was a Pb-p run (opposite direction). This makes the lab frame boost in the opposite direction. 
{
    ...
    for (int i = 0; i < numhists; i++) {
        harr[i] = new TH1D(Form("%ieta%i", runNumber, i), Form("%1.1f < #eta < %1.1f (#times %1.3f);#left|#it{Q}#right| #left[GeV/#it{c}#right];d^{2}#sigma/d#left|#it{Q}#right|dy #left[pb (GeV/#it{c})^{-1}#right]", eta_cuts[i], eta_cuts[i+1], harr_scales[i]), sizeof(xbins)/sizeof(xbins[0])-1, xbins);
