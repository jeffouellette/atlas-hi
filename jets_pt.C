             ...
             double luminosity) // Integrated luminosity for this run. Presumed constant over the run period.
{
    ...
    for (int i = 0; i < numentries; i++) {
        ...
        for (int j = 0; j < njet; j++) { // iterate over each jet momentum
            ...
            ebin--;
            if (ebin == -1 || ebin >= numetabins) continue; // Verify that the jet pseudorapidity was detectable.

            pbin = 0;
            while (pbins[pbin] <= jpt) pbin++;
            pbin--;
            if (pbin == -1 || pbin >= numpbins) continue; // Verify that the jet momentum was high enough to be in one of the bins.
