...
TH1D* GetDataOverMC(const TString name, TH2D* data, TH2D* mc, const int numxbins, const double* xbins, const int numybins, const double* ybins, const bool useFit) {
  ...
  for (int xbin = 1; xbin <= numxbins; xbin++) {
   ...
   if (useFit && useGaussian && numNonzeroBins > 4) {
    TF1* gaus = new TF1("gaus", "gaus(0)", projy->GetXaxis()->GetBinLowEdge(1), 2.0);
    projy->Fit(gaus, "Q0R");
  ...
  for (short iPer = 0; iPer < 3; iPer++) { // loop over period configurations
   ...
   for (short iEta = 0; iEta <= numetabins; iEta++) { // loop over bins in eta
    ...
    for (short iAlgo = 0; iAlgo < 2; iAlgo++) { // loop over jet algorithms
     ...
     jetRtrkHist_rat = GetDataOverMC(TString(Form("jetRtrk_DataMCRatio_%s_%s_iEta%i", algo.Data(), perType.Data(), iEta)), jetRtrkHists[iAlgo][iPer][iEta][0][1], jetRtrkHists[iAlgo][iPer][iEta][1][1], numpzbins, pzbins, numxjrefbins, xjrefbins, false);
     jetRtrkGraph_rat_sys = new TGraphAsymmErrors(jetRtrkHist_rat);
     jetRtrkHist_rat_lo = GetDataOverMC(TString(Form("jetRtrk_DataMCRatio_lo_%s_iEta%i", algo.Data(), iEta)), jetRtrkHists[iAlgo][iPer][iEta][0][0], jetRtrkHists[iAlgo][iPer][iEta][1][1], numpzbins, pzbins, numxjrefbins, xjrefbins, false);
     jetRtrkHist_rat_hi = GetDataOverMC(TString(Form("jetRtrk_DataMCRatio_hi_%s_iEta%i", algo.Data(), iEta)), jetRtrkHists[iAlgo][iPer][iEta][0][2], jetRtrkHists[iAlgo][iPer][iEta][1][1], numpzbins, pzbins, numxjrefbins, xjrefbins, false);
