...
void ZeeJetsHist () {
  ...
  SetupDirectories ("ZeeJets/", "pPb_8TeV_2016_jet_calibration/");
  ...
  TH3D***** zeeJetHists = Get4DArray <TH3D*> (2, 3, 3, 3); // iYear, iPer, iData, iErr
  //TH2D* zeeJetHistsSys[3][numetabins][3][3];
  TH2D**** zeeJetCounts = Get3DArray <TH2D*> (2, 3, 3);
   ...
   for (short iData = 0; iData < 3; iData++) { // iData is 0 for data, 1 for MC
    ...
    for (short iPer = 0; iPer < 3; iPer++) {
     ...
     for (short iErr = 0; iErr < 3; iErr++) {
      ...
      zeeJetHists[iYear][iPer][iData][iErr] = new TH3D (Form ("zeeJetPtRatio_%s_%s_%s_%s", year.Data (), dataType.Data (), error.Data (), period.Data ()), "", numpbins, pbins, numetabins, etabins, numxjrefbins, xjrefbins);
      zeeJetHists[iYear][iPer][iData][iErr]->Sumw2 ();
     }

     zeeJetCounts[iYear][iPer][iData] = new TH2D (Form ("zeeJetCounts_%s_%s_%s", year.Data (), dataType.Data (), period.Data ()), "", numpbins, pbins, numetabins, etabins);
  ...
  for (short iYear = 0; iYear < 2; iYear++) {
   ...
   while ( (sysfile= (TSystemFile*)next ())) {
    ...
    if (!sysfile->IsDirectory () && fname.EndsWith (".root")) {
     if (debugStatements) cout << "Status: In ZeeJetsHist.C: Found " << fname.Data () << endl;
  ...
  for (short iPer = 0; iPer < 3; iPer++) { // loop over period configurations
   ...
   for (short iEta = 0; iEta <= numetabins; iEta++) { // loop over bins in eta
    ...
    for (short iYear = 0; iYear < 2; iYear++) {
     ...
     const int countsData = zeeJetCounts[1][iPer][0]->Integral (1, zeeJetCounts[1][iPer][0]->GetNbinsX(), eta_lo, eta_hi);
     const int countsMC = zeeJetCounts[1][iPer][1]->Integral (1, zeeJetCounts[1][iPer][1]->GetNbinsX(), eta_lo, eta_hi);
     ...
     if (eta_lo != 1 || eta_hi != numetabins)
      myText (0.155, 0.15, dataColor, Form ("Z (ee) + Jet, %g < #eta_{det}^{Jet} < %g", etabins[eta_lo-1], etabins[eta_hi]), 0.04/uPadY);
     else
      myText (0.155, 0.15, dataColor, "Z (ee) + Jet", 0.04/uPadY);
     myText (0.155, 0.08, dataColor, period.Data (), 0.04/uPadY);

     bottomPad->cd ();
     bottomPad->SetLogx ();

     vJetHist_rat = GetDataOverMC (TString (Form ("zeeJetPtDataMCRatio_iEta%i", iEta)), proj, proj_mc_overlay, numpbins, pbins, false, "x");
     vJetHist_rat_lo = GetDataOverMC (TString (Form ("zeeJetPtDataMCRatio_lo_iEta%i", iEta)), proj_lo, proj_mc_overlay, numpbins, pbins, false, "x");
     vJetHist_rat_hi = GetDataOverMC (TString (Form ("zeeJetPtDataMCRatio_hi_iEta%i", iEta)), proj_hi, proj_mc_overlay, numpbins, pbins, false, "x");
   ...
   for (short iP = 0; iP <= numpbins; iP++) {
    ...
    for (short iYear = 0; iYear < 2; iYear++) {
     ...
     if (p_lo != 1 || p_hi != numpbins)
      myText (0.155, 0.15, dataColor, Form ("Z (ee) + Jet, %g < #it{p}_{T}^{ee} < %g", pbins[iP], pbins[iP+1]), 0.04/uPadY);
     else
      myText (0.155, 0.15, dataColor, "Z (ee) + Jet", 0.04/uPadY);
     myText (0.155, 0.08, dataColor, period.Data (), 0.04/uPadY);
