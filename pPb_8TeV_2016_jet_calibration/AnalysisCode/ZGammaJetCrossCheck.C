                          ...
                          const string inFileName = "") // Input root file name where tree is stored; if == "" code will try to guess file name based on other info
{
  ...
  for (long long entry = 0; entry < numEntries; entry++) {
   ...
   if (photon_n < 1 && muon_n < 2 && electron_n < 2) continue;

   // Z + jet -> e- + e+ + jet events
   if (electron_n >= 2) {
    for (int e1 = 0; e1 < electron_n; e1++) {
     if (electron_pt->at(e1) < electron_pt_cut) continue;
     if ((1.37 < TMath::Abs(electron_eta->at(e1)) &&
          TMath::Abs(electron_eta->at(e1)) < 1.52) ||
         2.47 < TMath::Abs(electron_eta->at(e1)))
      continue;
     for (int e2 = 0; e2 < e1; e2++) { 
      if (electron_pt->at(e2) < electron_pt_cut) continue;
      if ((1.37 < TMath::Abs(electron_eta->at(e2)) &&
           TMath::Abs(electron_eta->at(e2)) < 1.52) ||
          2.47 < TMath::Abs(electron_eta->at(e2))) continue;
      ...
      if (!(electron_loose->at(e1) && electron_loose->at(e2)))
      ...
      for (int jet = 0; jet < jet_n; jet++) {
       ...
       if (leading_jet == -1 ||
           jet_pt->at(leading_jet) < jet_pt->at(jet)) {
        subleading_jet = leading_jet;
        leading_jet = jet;
       }
       else if (subleading_jet == -1 ||
                jet_pt->at(subleading_jet) < jet_pt->at(jet)) {
        subleading_jet = jet;
       }
      }
   ...
   if (photon_n >= 1) {
    for (int p = 0; p < photon_n; p++) {
     ...
     if (!photon_tight->at(p))
      continue; // require tight cuts on photons

     if (photon_topoetcone40->at(p) > isolationEnergyIntercept + isolationEnergySlope*this_photon_pt)
