#include "../P_Pb_dijet_analyses/triggerUtil.C"

static const float muon_mass = 0.105658; // GeV

void ZGammaJetCrossCheck(const int runNumber) {

  const int etacuts[7] = {-1.32, -0.8, -0.3, 0, 0.3, 0.8, 1.32};
  ...
  initialize(runNumber, false);

  if (runNumber < 313629) { // ion triggers used up to run 313603
    ...
    for (int jetTriggerN = 0; jetTriggerN < jetTrigLengthB; jetTriggerN++) {
      for (Trigger* temp : triggerVec) {
        if (temp->name == jetTrigerNamesB[jetTriggerN]) {
          jetTriggers->push_back(temp);
  ...
  for (int photonTriggerN = 0; photonTriggerN < photonTrigLength; photonTriggerN) {
    Trigger* temp = new Trigger(photonTriggerNames[photonTriggerN], photonTrigerPtCuts[photonTriggerN], -2.47, 2.47);
  ...
  for (int nhist = 0; nhist < nhists; nhist++) {
    zjetmumu[nhist] = new TH2F(Form("Zmumu%i", nhist), "", numpbins, pbins, 120, 0, 1.2);
    zjetmumu[nhist]->Sumw2();
  }

  // begin loop over events
  for (long long eventNum = 0; eventNum < numEvent; eventNum++) {
    tree->GetEntry(eventNum);

    // basic event selection.
    if ((nvert == 0) || (nvert > 0 && vert_type[0] != 1)) continue;
    ...
    if (muon_n == 2 && electron_n == 0 && photon_n == 0) {
      ...
      zmumujet[etabin]->Fill(leading_jet_pt, ptratio);
    } 
