...
void RtrkHistMaker () {
  ...
  TH3D*** rtrkHists = Get2DArray <TH3D*> (2, 2); // iMC (0=signal, 1=overlay)
  for (short iTruth = 0; iTruth < 2; iTruth++) { // iTruth (0=reco., 1=truth)
    for (short iMC = 0; iMC < 2; iMC++) {
      rtrkHists[iMC][iTruth] = new TH3D (Form ("rtrkHist_%s_%s", iMC == 0 ? "signal":"overlay", iTruth == 0 ? "recotrks":"truthtrks"), "", numpbins, pbins, numetabins, etabins, numRtrkBins, rtrkBins);
      rtrkHists[iMC][iTruth]->Sumw2 ();
    }
  }
  ...
  Jet jet;
  float sum_trk_pt = 0, truth_sum_trk_pt = 0, evtWeight = 0;
  ...
  signalJets->SetBranchAddress ("truth_sum_trk_pt", &truth_sum_trk_pt);
  ...
  overlayJets->SetBranchAddress ("truth_sum_trk_pt",  &truth_sum_trk_pt);
  ...
  for (int iJet = 0; iJet < nSignalJets; iJet++) {
    ...
    rtrkHists[0][0]->Fill (jet.Pt, jet.Eta, sum_trk_pt / jet.Pt, evtWeight);
    rtrkHists[0][1]->Fill (jet.Pt, jet.Eta, truth_sum_trk_pt / jet.Pt, evtWeight);
    //rtrkHists[0]->Fill (jet.Pt, jet.Eta, sum_trk_pt, evtWeight);
  }
  for (int iJet = 0; iJet < nOverlayJets; iJet++) {
    overlayJets->GetEntry (iJet);
    rtrkHists[1][0]->Fill (jet.Pt, jet.Eta, sum_trk_pt / jet.Pt, evtWeight);
    rtrkHists[1][1]->Fill (jet.Pt, jet.Eta, truth_sum_trk_pt / jet.Pt, evtWeight);
  ...
  for (short iTruth = 0; iTruth < 2; iTruth++) {
    const int markerStyle = iTruth == 0 ? kFullCircle : kOpenCircle;

    TH2D* signalProj2d = Project2D (Form ("signalProj2d_%s", iTruth == 0 ? "recotrks":"truthtrks"), rtrkHists[0][iTruth], "x", "z", 1, numetabins);
    //signalProj2d->RebinY (20);
    TH1D* signalProf = GetProfileX (Form ("signalProf_%s", iTruth == 0 ? "recotrks":"truthtrks"), signalProj2d, numpbins, pbins, true, rtrk_los, rtrk_his);

    TH2D* overlayProj2d = Project2D (Form ("overlayProj2d_%s", iTruth == 0 ? "recotrks":"truthtrks"), rtrkHists[1][iTruth], "x", "z", 1, numetabins);
    //overlayProj2d->RebinY (20);
    TH1D* overlayProf = GetProfileX (Form ("overlayProf_%s", iTruth == 0 ? "recotrks":"truthtrks"), overlayProj2d, numpbins, pbins, true, rtrk_los, rtrk_his);
    ...
    signalProf->SetMarkerStyle (markerStyle);
    signalProf->SetMarkerSize (0.75);
    signalProf->SetLineColor (kBlue);
    signalProf->SetMarkerColor (kBlue);

    overlayProf->SetMarkerStyle (markerStyle);
    overlayProf->SetMarkerSize (0.75);
