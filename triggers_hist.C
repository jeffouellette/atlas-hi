...
void triggers_hist(int thisRunNumber) {
    ...
    TH2D* h2arr[numtrigs*numpbins];
    for (int pbin = 0; pbin < numpbins; pbin++) {
        for (Trigger* trig : trigger_vec) {
            int i = trig->index;
            h2arr[pbin*numtrigs + i] = (TH2D*)thisfile->Get(Form("pt%f_trig%i", pbins[pbin], i));
        }
    }
    ...
    h2_meta->GetZaxis()->SetLabelSize(0.016);
    h2_meta->GetZaxis()->SetTickLength(0.01);
    h2_meta->GetZaxis()->SetTitleOffset(0.1);
    h2_meta->Draw("COLZ, SAME");
/* TH2D* thishist;
    for (int pbin = 0; pbin < numpbins; pbin++) {
        for (Trigger* trig : trigger_vec) {
            int i = trig->index;
            if (thisRunNumber == 313063) {
                cout << Form("h2arr[%i] at pbin=%i, trig=%i with value %f", pbin*numtrigs+i, pbin, i, h2arr[pbin*numtrigs+i]->GetBinContent(i+1, pbin+1)) << endl;
            }
            thishist = h2arr[pbin*numtrigs + i];
            thishist->SetMinimum(minval);
            thishist->SetMaximum(maxval);

            thishist->GetXaxis()->SetLabelOffset(999);
            thishist->GetXaxis()->SetLabelSize(0);
            thishist->GetXaxis()->SetTickLength(0);
            thishist->GetXaxis()->SetTitleOffset(1.3);

            thishist->GetYaxis()->SetLabelOffset(999);
            thishist->GetYaxis()->SetLabelSize(0);    
            thishist->GetYaxis()->SetTickLength(0);
            thishist->GetYaxis()->SetTitleOffset(1.2);

            
            thishist->GetZaxis()->SetLabelOffset(999);
            thishist->GetZaxis()->SetLabelSize(0);
            thishist->GetZaxis()->SetTickLength(0);
            thishist->Draw("COLZ, SAME");
        }
    }
    h2arr[0]->GetZaxis()->SetLabelSize(0.016);
    h2arr[0]->GetZaxis()->SetTitleOffset(0.1);
*/
    for (Trigger* trig : trigger_vec) {
        int i = trig->index;
