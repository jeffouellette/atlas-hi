...
%  txfonts=true|false    Use txfonts rather than the default newtxw
...
\input{pPbJetCalibrationNote-metadata}
% Author and title for the PDF file
\hypersetup{pdftitle={ATLAS document},pdfauthor={The ATLAS Collaboration}}
\AtlasAuthorContributor{Jeffrey Ouellette}{a}{}
...
%, and (2) the ratio of track momenta within the jet cone to the calorimeter momentum. 
...
\begin{figure}[htbp]
	...
	\caption{$\eta$ closure in each collision period for R=0.4 jets.}
...
\begin{figure}[htbp]
	\centering
	\subfloat[]{
		\includegraphics[width=0.45\textwidth]{figures/CompareEtaInterCcalibration.pdf}
		\label{fig:insituEtaComp}
	}
	\subfloat[]{
		\includegraphics[width=0.45\textwidth]{figures/CompareSummary.pdf}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.9\textwidth]{figures/xcalib_map.pdf}
...
%The ATLAS tracker has sufficient precision to compare the $\pt$ of tracks in the jet cone to the $\pt$ of the entire jet and is also used to corroborate the $\pt$-balance results. Here, only $\gamma$ + jet events are used for simplicity. Events are selected the same as before, but the jets are only required to match to the candidate photon. Additionally, tracks within the jet cone are selected at the ``TightPrimary'' level and are required to have $\pt \geq \SI{1}{\GeV}$. Due to limitations of the tracker coverage, $R=0.4$ jets are only considered within $\left|\eta\right| < 2.1$.
%
%In these events, the quantity $\left<R_{trk}\right> \equiv \left<\Sigma p_{T}^{trk} / p_{T}^{Calo}\right>$ is calculated as a function of $p_{T}^{Calo}$ and is compared between the data and MC. The results are shown in Figure \ref{fig:rtrk_summary}. In the combined period A \& B result, any difference between data and MC is shown to be at the level $0.7\%$ at most, which is consistent with the previous result. However, uncertainties in the jet energy scale are expected to dominate over this; for instance, the jet energy closure in MC statistically fluctuates at the level of $1\%$, as seen in Figure \ref{fig:EnergyClosure}. This study additionally suggests that the cross-calibration is applicable to the \textit{p}+Pb data, at this level of precision.
%
%A known issue with the \textit{p}+Pb tracking in MC overlay suggests the $R{trk}$ study should not be completely relied upon, however it still supports the $\pt$ balance results.

\begin{figure}[htbp]
	\centering
	\subfloat[]{
		\includegraphics[width=0.45\textwidth]{figures/v_jet_xjref_PAB_iEta_combined.pdf}
		\label{fig:v_xjref_pAB_pT}
	}
	\subfloat[]{
		\includegraphics[width=0.45\textwidth]{figures/v_jet_dataOverMC_PAB_iEta_combined.pdf}
		\label{fig:v_xjref_dataOverMC_pAB_pT}
	}\\
	\subfloat[]{
		\includegraphics[width=0.45\textwidth]{figures/v_jet_xjref_PA_iEta_combined.pdf}
		\label{fig:v_xjref_pA_pT}
	}
	\subfloat[]{
		\includegraphics[width=0.45\textwidth]{figures/v_jet_dataOverMC_PA_iEta_combined.pdf}
		\label{fig:v_xjref_dataOverMC_pA_pT}
	}\\
	\subfloat[]{
		\includegraphics[width=0.45\textwidth]{figures/v_jet_xjref_PB_iEta_combined.pdf}
		\label{fig:v_xjref_pB_pT}
	}
	\subfloat[]{
		\includegraphics[width=0.45\textwidth]{figures/v_jet_dataOverMC_PB_iEta_combined.pdf}
		\label{fig:v_xjref_dataOverMC_pB_pT}
	}
...
\begin{figure}[htbp]
	\centering
	\subfloat[]{
		\includegraphics[width=0.45\textwidth]{figures/v_jet_xjref_PAB_iP_combined.pdf}
		\label{fig:v_xjref_pAB_eta}
	}
	\subfloat[]{
		\includegraphics[width=0.45\textwidth]{figures/v_jet_dataOverMC_PAB_iP_combined.pdf}
		\label{fig:v_xjref_dataOverMC_pAB_eta}
	}\\
	\subfloat[]{
		\includegraphics[width=0.45\textwidth]{figures/v_jet_xjref_PA_iP_combined.pdf}
		\label{fig:v_xjref_pA_eta}
	}
	\subfloat[]{
		\includegraphics[width=0.45\textwidth]{figures/v_jet_dataOverMC_PA_iP_combined.pdf}
		\label{fig:v_xjref_dataOverMC_pA_eta}
	}\\
	\subfloat[]{
		\includegraphics[width=0.45\textwidth]{figures/v_jet_xjref_PB_iP_combined.pdf}
		\label{fig:v_xjref_pB_eta}
	}
	\subfloat[]{
		\includegraphics[width=0.45\textwidth]{figures/v_jet_dataOverMC_PB_iP_combined.pdf}
		\label{fig:v_xjref_dataOverMC_pB_eta}
	}
...
%\begin{figure}
%	\centering
%	\includegraphics[width=0.9\textwidth]{figures/jet_rtrk_PAB_iEta_combined.pdf}
%	\caption{Average $\left<R_{trk}\right>$ as a function of jet (calorimeter) $p_{T}$ in both period A and period B collisions. In the bottom panel, the data-to-MC ratio is shown along with its average value, which suggests that data and MC are different at the level of $0.7\%$.}
...
%A study of the tracks within the jet cone shows similar behavior, supporting this conclusion.
