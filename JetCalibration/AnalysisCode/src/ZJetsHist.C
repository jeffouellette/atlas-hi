...
void ZJetsHist () {
  ...
  SetupDirectories ("ZJets/", "JetCalibration/");

  //////////////////////////////////////////////////////////////////////////////
  // Create desired histograms
  //////////////////////////////////////////////////////////////////////////////
  TH3D**** zJetHists_pt_eta = Get3DArray <TH3D*> (3, 2, 3); // iPer, iData, iErr
  TH3D*** zJetCounts = Get2DArray <TH3D*> (3, 2); // iPer, iData
  ...
  for (short iPer = 0; iPer < 3; iPer++) {
    ...
    for (short iData = 0; iData < 2; iData++) { // iData is 0 for data, 1 for MC
      ...
      for (short iErr = 0; iErr < 3; iErr++) {
        ...
        const char* name = Form ("zJetPtRatio_%s_%s_%s", per, data, error);
        zJetHists_pt_eta[iPer][iData][iErr] = new TH3D (name, "", numpbins, pbins, numetabins, etabins, 1000, linspace (0, 2.0, 1000));
        zJetHists_pt_eta[iPer][iData][iErr]->Sumw2 ();
      }

      const char* name = Form ("zJetCounts_%s_%s", per, data);
      zJetCounts[iPer][iData] = new TH3D (name, "", numpbins, pbins, numetabins, etabins, numphibins, phibins);
      zJetCounts[iPer][iData]->Sumw2 ();
    }
  }
  ...
  for (int zJet = 0; zJet < nZJets; zJet++) {
    ...
    if (zpt > 0) {
      ...
      if (!isMC) {
        zJetHists_pt_eta[(short)isPeriodA][0][0]->Fill (zpt, jeta, xjref-xjreferr, evtWeight);
        zJetHists_pt_eta[(short)isPeriodA][0][2]->Fill (zpt, jeta, xjref+xjreferr, evtWeight);
        zJetHists_pt_eta[2][0][0]->Fill (zpt, jeta, xjref-xjreferr, evtWeight);
        zJetHists_pt_eta[2][0][2]->Fill (zpt, jeta, xjref+xjreferr, evtWeight);
      }
    }

    zJetCounts[(short)isPeriodA][(short)isMC]->Fill (zpt, jeta, jphi);
    zJetCounts[2][(short)isMC]->Fill (zpt, jeta, jphi);
  }
  inTree = NULL;
  inFile->Close ();
  ...
  for (short iPer = 0; iPer < 3; iPer++) {
    for (short iData = 0; iData < 2; iData++) { // iData is 0 for data, 1 for MC
      for (short iErr = 0; iErr < 3; iErr++) {
        if (iErr != 1 && iData != 0)
          continue;
        zJetHists_pt_eta[iPer][iData][iErr]->Write ();
      }
      zJetCounts[iPer][iData]->Write ();
    }
  }
  outFile->Close ();
  ...
  for (short iPer = 0; iPer < 3; iPer++) { // loop over period configurations
    ...
    for (short iEta = 0; iEta <= numetabins; iEta++) { // loop over bins in eta
      ...
      proj = Project2D ("", zJetHists_pt_eta[iPer][0][1], "x", "z", eta_lo, eta_hi, exclusive && iEta == numetabins);
      ...
      proj_hi = Project2D ("", zJetHists_pt_eta[iPer][0][2], "x", "z", eta_lo, eta_hi, exclusive && iEta == numetabins);
      ...
      proj_mc = Project2D ("", zJetHists_pt_eta[iPer][1][1], "x", "z", eta_lo, eta_hi, exclusive && iEta == numetabins);
      ...
      if (exclusive && iEta == numetabins) {
        countsData = zJetCounts[iPer][0]->Integral () - zJetCounts[iPer][0]->Integral (1, zJetCounts[iPer][0]->GetNbinsX(), eta_lo, eta_hi, 1, numphibins);
        countsMC = zJetCounts[iPer][1]->Integral () - zJetCounts[iPer][1]->Integral (1, zJetCounts[iPer][1]->GetNbinsX(), eta_lo, eta_hi, 1, numphibins);
      }
      else {
        countsData = zJetCounts[iPer][0]->Integral (1, zJetCounts[iPer][0]->GetNbinsX(), eta_lo, eta_hi, 1, numphibins);
        countsMC = zJetCounts[iPer][1]->Integral (1, zJetCounts[iPer][1]->GetNbinsX(), eta_lo, eta_hi, 1, numphibins);
      }
      ...
      if (eta_lo != 1 || eta_hi != numetabins)
        if (!(exclusive && iEta == numetabins))
          myText (0.155, 0.135, kBlack, Form ("Z + Jet, %g < #eta_{det}^{Jet} < %g", etabins[eta_lo-1], etabins[eta_hi]), 0.032/uPadY);
        else
          myText (0.155, 0.135, kBlack, Form ("Z + Jet, %g < #left|#eta_{det}^{Jet}#right|", etabins[eta_hi]), 0.032/uPadY);
      else
        myText (0.155, 0.14, kBlack, "Z + Jet", 0.032/uPadY);
      myText (0.155, 0.08, kBlack, period, 0.032/uPadY);

      bottomPad->cd ();
      bottomPad->SetLogx ();

      vJetHist_rat = GetDataOverMC (Form ("zJetPtDataMCRatio_iEta%i", iEta), proj, proj_mc, numpbins, pbins, true, "x");
      vJetHist_rat_lo = GetDataOverMC (Form ("zJetPtDataMCRatio_lo_iEta%i", iEta), proj_lo, proj_mc, numpbins, pbins, true, "x");
      vJetHist_rat_hi = GetDataOverMC (Form ("zJetPtDataMCRatio_hi_iEta%i", iEta), proj_hi, proj_mc, numpbins, pbins, true, "x");
    ...
    for (short iP = 0; iP <= numpbins; iP++) {
      ...
      proj = Project2D ("", zJetHists_pt_eta[iPer][0][1], "y", "z", p_lo, p_hi);
      proj->RebinY (rebinFactor);
      ...
      proj_hi = Project2D ("", zJetHists_pt_eta[iPer][0][2], "y", "z", p_lo, p_hi);
      proj_hi->RebinY (rebinFactor);
      ...
      proj_mc = Project2D ("", zJetHists_pt_eta[iPer][1][1], "y", "z", p_lo, p_hi);
      proj_mc->RebinY (rebinFactor);
      ...
      const int countsMC = zJetCounts[iPer][1]->Integral (p_lo, p_hi, 1, zJetCounts[iPer][1]->GetNbinsY(), 1, numphibins);

      myMarkerText (0.175, 0.88, dataColor, dataStyle, Form ("2016 Data (%i events)", countsData), 1.25, 0.032/uPadY);
      myMarkerText (0.175, 0.82, mcColor, mcStyle, Form ("Pythia8 MC (%i events)", countsMC), 1.25, 0.032/uPadY);
      myText (0.155, 0.2, kBlack, "#bf{#it{ATLAS}} Internal", 0.032/uPadY);
      if (p_lo != 1 || p_hi != numpbins)
        myText (0.155, 0.135, kBlack, Form ("Z + Jet, %g < #it{p}_{T}^{Z} < %g", pbins[p_lo-1], pbins[p_hi]), 0.032/uPadY);
      else
        myText (0.155, 0.14, kBlack, "Z + Jet", 0.032/uPadY);
      myText (0.155, 0.08, kBlack, period, 0.032/uPadY);
      ...
      vJetHist_rat = GetDataOverMC (Form ("zJetPtDataMCRatio_iP%i", iP), proj, proj_mc, numetabins, etabins, true, "x");
      vJetHist_rat_lo = GetDataOverMC (Form ("zJetPtDataMCRatio_lo_iP%i", iP), proj_lo, proj_mc, numetabins, etabins, true, "x");
      vJetHist_rat_hi = GetDataOverMC (Form ("zJetPtDataMCRatio_hi_iP%i", iP), proj_hi, proj_mc, numetabins, etabins, true, "x");
    ...
    for (short iEta = 0; iEta <= numetabins; iEta++) { // loop over bins in eta
      ...
      for (short iP = 0; iP <= numpbins; iP++) {
        ...
        myText (0.155, 0.43, dataColor, Form ("Z + Jet, %s", period), 0.04);
        ...
        //if (iEta == 0 && iP == 8) {
        //  myText (0.155, 0.43, dataColor, Form ("Z + Jet, %s", period), 0.04/gPad->GetHNDC ());
