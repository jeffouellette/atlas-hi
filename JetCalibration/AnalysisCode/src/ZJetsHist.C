  ...
  for (short iPer = 0; iPer < 3; iPer++) { // loop over period configurations
    ...
    for (short iEta = 0; iEta <= numetabins; iEta++) { // loop over bins in eta
      ...
      if (eta_lo != 1 || eta_hi != numetabins) {
        if (!(exclusive && iEta == numetabins))
          myText (0.195, 0.135, kBlack, Form ("Z + Jet, %g < #eta_{det}^{Jet} < %g", etabins[eta_lo-1], etabins[eta_hi]), 0.032/uPadY);
        else
          myText (0.195, 0.135, kBlack, Form ("Z + Jet, %g < #left|#eta_{det}^{Jet}#right|", etabins[eta_hi]), 0.032/uPadY);
      }
      else
        myText (0.195, 0.14, kBlack, "Z + Jet", 0.032/uPadY);
      myText (0.195, 0.08, kBlack, period, 0.032/uPadY);

      bottomPad->cd ();
      bottomPad->SetLogx ();

      vJetHist_rat = (TH1D*) vJetHist->Clone ("zJetPtDataMCRatio");
      vJetHist_rat_lo = (TH1D*) vJetHist_lo->Clone ("zJetPtDataMCRatio_lo");
      vJetHist_rat_hi = (TH1D*) vJetHist_hi->Clone ("zJetPtDataMCRatio_hi");
      ...
      //  myText (0.195, 0.4, kBlack, Form ("#chi^{2} = %g", chisq), 0.032/dPadY);
    ...
    for (short iP = 0; iP <= numpbins; iP++) {
      ...
      const int countsData = zJetCounts[iPer][0]->Integral (p_lo, p_hi, 1, numetabins, 1, numphibins);
      const int countsMC = zJetCounts[iPer][1]->Integral (p_lo, p_hi, 1, numetabins, 1, numphibins);

      myMarkerText (0.215, 0.88, dataColor, dataStyle, Form ("2016 Data (%i events)", countsData), 1.25, 0.032/uPadY);
      myMarkerText (0.215, 0.82, mcColor, mcStyle, Form ("Pythia8 MC + Overlay (%i events)", countsMC), 1.25, 0.032/uPadY);
      myText (0.195, 0.2, kBlack, "#bf{#it{ATLAS}} Internal", 0.032/uPadY);
      if (p_lo != 1 || p_hi != numpbins)
        myText (0.195, 0.135, kBlack, Form ("Z + Jet, %g < #it{p}_{T}^{Z} < %g", pbins[p_lo-1], pbins[p_hi]), 0.032/uPadY);
      else
        myText (0.195, 0.14, kBlack, "Z + Jet", 0.032/uPadY);
      myText (0.195, 0.08, kBlack, period, 0.032/uPadY);
    ...
    for (short iEta = 0; iEta <= numetabins; iEta++) { // loop over bins in eta
      ...
      for (short iP = 0; iP <= numpbins; iP++) {
        ...
        myText (0.655, 0.88, dataColor, Form ("Z + Jet, %s", period), 0.032);
