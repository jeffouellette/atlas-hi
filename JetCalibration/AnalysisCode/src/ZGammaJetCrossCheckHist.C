...
void ZGammaJetCrossCheckHist () {
  ...
  TGraphErrors* constFit = new TGraphErrors ("constFit");
  constFit->SetFillColorAlpha (2, 0.3);
  constFit->SetLineColorAlpha (2, 0.3);
  constFit->SetFillStyle (1001);
   ...
   for (short iEta = 0; iEta <= numetabins; iEta++) { // loop over bins in eta
    ...
    for (short iYear = 0; iYear < 2; iYear++) {
     ...
     for (int iMC = 1; iMC < 3; iMC++) { // loops over overlay then signal MC
      ...
      vJetHist_rat->SetXTitle ("#it{p}_{T}^{V} #left[GeV#right]");
      ...
      TF1* midConst = new TF1 ("midConst", "[0]", 0, pbins[numpbins]);
      vJetHist_rat->Fit (midConst, "NQ0R");
      constFit->SetPoint (0, pbins[numpbins] / 2, midConst->GetParameter (0));
      constFit->SetPointError (0, pbins[numpbins] / 2, midConst->GetParError (0));
      ...
      if (midConst) { delete midConst; midConst = NULL; }
     }
