...
void AddWeighted (TGraphAsymmErrors** num, TGraphAsymmErrors** den, const TGraphAsymmErrors* n, const TGraphAsymmErrors* nsys) {
  ...
  for (int ix = 0; ix < n->GetN (); ix++) {
   ...
   ne = nsys->GetErrorY (ix);
   ei = num[0]->GetErrorY (ix);
   num[0]->GetPoint (ix, x, y);
   num[0]->SetPoint (ix, nx, y + w*ne);
...
TGraphAsymmErrors* Divide (const TString name, const TGraphAsymmErrors* num, const TGraphAsymmErrors* den) {
  TGraphAsymmErrors* rat = new TGraphAsymmErrors (name);
...
void ZGammaJetCrossCheckHist () {
  ...
  TH3D****** vJetHists = Get5DArray <TH3D*> (2, 3, 3, 3, 4);
  TH2D***** vJetCounts = Get4DArray <TH2D*> (2, 3, 3, 4);
    ...
    for (short iPer = 0; iPer < 3; iPer++) {
     ...
     for (short iSpc = 0; iSpc < 4; iSpc++) {
      ...
      for (short iErr = 0; iErr < 3; iErr++) {
       ...
       vJetHists[iYear][iPer][iData][iErr][iSpc] = (TH3D*)inFile->Get (name);
      }
 
      const TString name = Form ("%sJetCounts_%s_%s_%s", spc.Data (), year.Data (), dataType.Data (), period.Data ());
      vJetCounts[iYear][iPer][iData][iSpc] = (TH2D*)inFile->Get (name);
     }
    }
   }
  }
  ...
  TGraphAsymmErrors** combinedNum = NULL; // 0 = sys, 1 = stat
  ...
  for (short iPer = 0; iPer < 3; iPer++) { // loop over period configurations
   ...
   for (short iEta = 0; iEta <= numetabins; iEta++) { // loop over bins in eta
    ...
    for (short iYear = 0; iYear < 2; iYear++) {
     ...
     combinedNum = Get1DArray <TGraphAsymmErrors*> (2); // 0 = sys, 1 = stat
     combinedDen = Get1DArray <TGraphAsymmErrors*> (2);
     combinedRatio = Get1DArray <TGraphAsymmErrors*> (2); // 0 = sys, 1 = stat)

     for (short iErr = 0; iErr < 2; iErr++) {
      const TString error = (iErr == 0 ? "sys" : "stat");
      combinedNum[iErr] = new TGraphAsymmErrors (Form ("combinedNum_%s", error.Data ()));
      combinedDen[iErr] = new TGraphAsymmErrors (Form ("combinedDen_%s", error.Data ()));
     }
     ...
     for (short iSpc = 0; iSpc < 3; iSpc++) {
      ...
      // proj_mc_signal = Project2D ("", vJetHists[iYear][iPer][2][1][iSpc], "x", "z", eta_lo, eta_hi);
      ...
      if (exclusive && iEta == numetabins) {
       countsData = vJetCounts[iYear][iPer][0][iSpc]->Integral () - vJetCounts[iYear][iPer][0][iSpc]->Integral (1, vJetCounts[iYear][iPer][0][iSpc]->GetNbinsX(), eta_lo, eta_hi);
       countsMC = vJetCounts[iYear][iPer][1][iSpc]->Integral () - vJetCounts[iYear][iPer][1][iSpc]->Integral (1, vJetCounts[iYear][iPer][1][iSpc]->GetNbinsX(), eta_lo, eta_hi);
       //if (!skipSignalMC)
       // countsMC_sig = vJetCounts[iYear][iPer][2][iSpc]->Integral () - vJetCounts[iYear][iPer][2][iSpc]->Integral (1, vJetCounts[iYear][iPer][2][iSpc]->GetNbinsX(), eta_lo, eta_hi);
      }
      else {
       countsData = vJetCounts[iYear][iPer][0][iSpc]->Integral (1, vJetCounts[iYear][iPer][0][iSpc]->GetNbinsX(), eta_lo, eta_hi);
       countsMC = vJetCounts[iYear][iPer][1][iSpc]->Integral (1, vJetCounts[iYear][iPer][1][iSpc]->GetNbinsX(), eta_lo, eta_hi);
       //if (!skipSignalMC)
       // countsMC_sig = vJetCounts[iYear][iPer][2][iSpc]->Integral (1, vJetCounts[iYear][iPer][2][iSpc]->GetNbinsX(), eta_lo, eta_hi);
      }
      ...
      for (int iMC = 1; iMC < 3; iMC++) { // loops over overlay then signal MC
       ...
       AddWeighted (combinedNum, combinedDen, vJetGraph_rat, vJetGraph_sys);
       ...
       vJetGraph_rat->GetYaxis ()->SetTitle ("<x_{J}^{ref}> Data / MC");
       ...
       if ((skipOldInsitu || iYear == 0) && iMC == 1 && iSpc == 0)
     ...
     for (short iErr = 0; iErr < 2; iErr++) { 
      const TString error = (iErr == 0 ? "sys" : "stat");
      combinedRatio[iErr] = Divide (Form ("combinedRatio_%s", error.Data ()), combinedNum[iErr], combinedDen[iErr]);
     }
     for (int ix = 0; ix < combinedRatio[0]->GetN (); ix++) {
      double x, y, ex, ey;
      combinedRatio[0]->GetPoint (ix, ex, ey);
      combinedRatio[1]->GetPoint (ix, x, y);
      combinedRatio[0]->SetPoint (ix, x, y);
      combinedRatio[0]->SetPointError (ix, combinedRatio[1]->GetErrorXlow (ix), combinedRatio[1]->GetErrorXhigh (ix), ey, ey);
     }
     for (short iErr = 0; iErr < 2; iErr++) { 
     ...
     Delete1DArray (combinedNum, 2);
     Delete1DArray (combinedDen, 2);
   ...
   for (short iP = 0; iP <= numpbins; iP++) {
    ...
    for (short iYear = 0; iYear < 2; iYear++) {
     ...
     for (short iSpc = 0; iSpc < 3; iSpc++) {
      ...
      //if (iPer != 1 && !skipSignalMC) {
      // proj_mc_signal = Project2D ("", vJetHists[iYear][iPer][2][1][iSpc], "y", "z", p_lo, p_hi);
      // vJetHist_mc_signal = GetProfileX ("vJetHist_mc_signal", proj_mc_signal, numpbins, pbins, true);
      ...
      int countsData = 0, countsMC = 0;//, countsMC_sig = 0;
      countsData = vJetCounts[iYear][iPer][0][iSpc]->Integral (p_lo, p_hi, 1, vJetCounts[iYear][iPer][0][iSpc]->GetNbinsY());
      countsMC = vJetCounts[iYear][iPer][1][iSpc]->Integral (p_lo, p_hi, 1, vJetCounts[iYear][iPer][1][iSpc]->GetNbinsY());
      //if (!skipSignalMC)
      // countsMC_sig = vJetCounts[iYear][iPer][2][iSpc]->Integral (p_lo, p_hi, 1, vJetCounts[iYear][iPer][2][iSpc]->GetNbinsY());
      ...
      for (int iMC = 1; iMC < 3; iMC++) { // loops over overlay then signal MC
       ...
       if ((skipOldInsitu || iYear == 0) && iMC == 1 && iSpc == 0)
        ...
        //vJetHist_rat->DrawCopy ("same e1 x0");
        ( (TGraphAsymmErrors*)vJetGraph_rat->Clone ())->Draw ("p");
       ( (TGraphAsymmErrors*)vJetGraph_rat_sys->Clone ())->Draw ("2");
