  ...
  for (short iAlgo = 0; iAlgo < 2; iAlgo++) {
   ...
   for (short iMC = plotMC; iMC < plotMC+2; iMC++) {
    ...
    for (short iEta = 0; iEta < 2; iEta++) {
     for (int iP = 0; iP < numpbins; iP++) {
      TH1D* thisHist = jetEnergyResponseEta[iEta][iP];
      ...
      recoFit = new TF1 ("recoFit2", "gaus (0)", m - 2.0*s, m + 2.0*s);
      recoFit->SetParameter (1, thisHist->GetMean ());
      recoFit->SetParameter (2, thisHist->GetStdDev ());
      thisHist->Fit (recoFit, "RN");

      jetEnergyScale[iMC][iEta]->SetBinContent (iP+1, recoFit->GetParameter (1));

      jetEnergyScale[iMC][iEta]->SetBinError (iP+1, recoFit->GetParError (1));

      jetEnergyRes[iMC][iEta]->SetBinContent (iP+1, recoFit->GetParameter (2));

      jetEnergyRes[iMC][iEta]->SetBinError (iP+1, recoFit->GetParError (2));

      if (recoFit) delete recoFit;
     }
    }
    Delete2DArray (jetEnergyResponseEta, 2, numpbins);
   }
   ...
   for (short iMC = plotMC; iMC < plotMC+2; iMC++) {
    ...
    for (int iEta = 0; iEta <= numetabins; iEta++) {
     if (jetEnergyScale[iMC][iEta]->Integral () == 0)
      continue;
     ...
     if (iEta == 0)
      myMarkerText (0.22, 0.25, colors[iEta], kFullCircle, "|#eta| < 1.2", 1.25, 0.03);
     else if (iEta == 1)
      myMarkerText (0.22, 0.21, colors[iEta], kFullCircle, "|#eta| > 1.2", 1.25, 0.03);
