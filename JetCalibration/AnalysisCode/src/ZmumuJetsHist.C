   ...
   for (short iPer = 0; iPer < 3; iPer++) {
    ...
    for (short iData = 0; iData < 2; iData++) { // iData is 0 for data, 1 for MC
     ...
     for (short iErr = 0; iErr < 3; iErr++) {
      ...
      zmumuJetHists[iYear][iPer][iData][iErr] = (TH3D*)inFile->Get (name);
     }

     const TString name = Form ("zmumuJetCounts_%s_%s_%s", year.Data (), dataType.Data (), period.Data ());
     zmumuJetCounts[iYear][iPer][iData] = (TH2D*)inFile->Get (name); 
    }
   }
  }
  ...
  for (short iPer = 0; iPer < 3; iPer++) { // loop over period configurations
   ...
   for (short iEta = 0; iEta <= numetabins; iEta++) { // loop over bins in eta
    ...
    for (short iYear = 0; iYear < 2; iYear++) {
     ...
     int countsData, countsMC;
     if (exclusive && iEta == numetabins) {
      countsData = zmumuJetCounts[1][iPer][0]->Integral () - zmumuJetCounts[1][iPer][0]->Integral (1, zmumuJetCounts[1][iPer][0]->GetNbinsX(), eta_lo, eta_hi);
      countsMC = zmumuJetCounts[1][iPer][1]->Integral () - zmumuJetCounts[1][iPer][1]->Integral (1, zmumuJetCounts[1][iPer][1]->GetNbinsX(), eta_lo, eta_hi);
     }
     else {
      countsData = zmumuJetCounts[1][iPer][0]->Integral (1, zmumuJetCounts[1][iPer][0]->GetNbinsX(), eta_lo, eta_hi);
