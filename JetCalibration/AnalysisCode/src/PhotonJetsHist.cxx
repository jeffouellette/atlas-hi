...
void PhotonJetsHist () {
  ...
  for (short iPer = 0; iPer < 3; iPer++) { // loop over period configurations
    ...
    for (short iP = 0; iP <= numpbins; iP++) {
      ...
      const TString func = "[0] + [1]*((x-[2])/[3])";// + [2]*(2*((x-[4])/[5])^2-1) + [3]*(4*((x-[4])/[5])^3-3*(x-[4])/[5])";// + [4]*(8*((x-[5])/[6])^4-8*((x-[5])/[6])^2+1)";
      ...
      vJetRatioFit->FixParameter (2, 0.5 * (etabins[numetabins] + etabins[0]));
      vJetRatioFit->FixParameter (3, 0.5 * (etabins[numetabins] - etabins[0]));
      vJetHist_rat->Fit (vJetRatioFit, "RN0");
