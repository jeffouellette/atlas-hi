    ...
    for (short iYear = 0; iYear < 2; iYear++) {
     ...
     if (eta_lo != 1 || eta_hi != numetabins)
      if (!(exclusive && iEta == numetabins))
       ...
       myText (0.155, 0.15, dataColor, Form ("Z (ee) + Jet, %g < #left|#eta_{det}^{Jet}#right|", etabins[eta_hi]), 0.04/uPadY);
     else
   ...
   for (short iP = 0; iP <= numpbins; iP++) {
    ...
    for (short iYear = 0; iYear < 2; iYear++) {
     ...
     if (p_lo != 1 || p_hi != numpbins)
      myText (0.155, 0.15, dataColor, Form ("Z (ee) + Jet, %g < #it{p}_{T}^{Z} < %g", pbins[p_lo-1], pbins[p_hi]), 0.04/uPadY);
     else
      myText (0.155, 0.15, dataColor, "Z (ee) + Jet", 0.04/uPadY);
     myText (0.155, 0.08, dataColor, period.Data (), 0.04/uPadY);
