...
void ZeeJetsHist () {
  ...
  //////////////////////////////////////////////////////////////////////////////
  TH3D**** zeeJetHists_pt_eta = Get3DArray <TH3D*> (3, 2, 3); // iPer, iData, iErr
  TH3D*** zeeJetCounts = Get2DArray <TH3D*> (3, 2); // iPer, iData
  ...
  for (short iPer = 0; iPer < 3; iPer++) {
    ...
    for (short iData = 0; iData < 2; iData++) { // iData is 0 for data, 1 for MC
      ...
      for (short iErr = 0; iErr < 3; iErr++) {
        ...
        const char* name = Form ("zeeJetPtRatio_%s_%s_%s", per, data, error);
        zeeJetHists_pt_eta[iPer][iData][iErr] = new TH3D (name, "", numpbins, pbins, numetabins, etabins, 1000, linspace (0, 2.0, 1000));
        zeeJetHists_pt_eta[iPer][iData][iErr]->Sumw2 ();
      }

      const char* name = Form ("zeeJetCounts_%s_%s", per, data);
      zeeJetCounts[iPer][iData] = new TH3D (name, "", numpbins, pbins, numetabins, etabins, numphibins, phibins);
      zeeJetCounts[iPer][iData]->Sumw2 ();
    }
  }
  ...
  for (int zJet = 0; zJet < nZJets; zJet++) {
    ...
    if (zpt > 0) {
      ...
      zeeJetHists_pt_eta[isPeriodA?0:1][isMC?1:0][1]->Fill (zpt, jeta, xjref, evtWeight);
      zeeJetHists_pt_eta[2][isMC?1:0][1]->Fill (zpt, jeta, xjref, evtWeight);

      if (!isMC) {
        zeeJetHists_pt_eta[isPeriodA?0:1][0][0]->Fill (zpt, jeta, xjref-xjreferr, evtWeight);
        zeeJetHists_pt_eta[isPeriodA?0:1][0][2]->Fill (zpt, jeta, xjref+xjreferr, evtWeight);
        zeeJetHists_pt_eta[2][0][0]->Fill (zpt, jeta, xjref-xjreferr, evtWeight);
        zeeJetHists_pt_eta[2][0][2]->Fill (zpt, jeta, xjref+xjreferr, evtWeight);
      }
    }

    zeeJetCounts[isPeriodA?0:1][isMC?1:0]->Fill (zpt, jeta, jphi);
    zeeJetCounts[2][isMC?1:0]->Fill (zpt, jeta, jphi);
  }
  inTree = NULL;
  inFile->Close ();
  ...
  for (short iPer = 0; iPer < 3; iPer++) {
    for (short iData = 0; iData < 2; iData++) { // iData is 0 for data, 1 for MC
      for (short iErr = 0; iErr < 3; iErr++) {
        if (iErr != 1 && iData != 0)
          continue;
        zeeJetHists_pt_eta[iPer][iData][iErr]->Write ();
      }
      zeeJetCounts[iPer][iData]->Write ();
    }
  }
  outFile->Close ();
  ...
  for (short iPer = 0; iPer < 3; iPer++) { // loop over period configurations
    ...
    for (short iEta = 0; iEta <= numetabins; iEta++) { // loop over bins in eta
      ...
      proj = Project2D ("", zeeJetHists_pt_eta[iPer][0][1], "x", "z", eta_lo, eta_hi, exclusive && iEta == numetabins);
      ...
      proj_lo = Project2D ("", zeeJetHists_pt_eta[iPer][0][0], "x", "z", eta_lo, eta_hi, exclusive && iEta == numetabins);
      proj_lo->RebinY (rebinFactor);
      vJetHist_lo = GetProfileX ("vJetHist_lo", proj_lo, numpbins, pbins, true);

      proj_hi = Project2D ("", zeeJetHists_pt_eta[iPer][0][2], "x", "z", eta_lo, eta_hi, exclusive && iEta == numetabins);
      ...
      proj_mc = Project2D ("", zeeJetHists_pt_eta[iPer][1][1], "x", "z", eta_lo, eta_hi, exclusive && iEta == numetabins);
      ...
      if (exclusive && iEta == numetabins) {
        countsData = zeeJetCounts[iPer][0]->Integral () - zeeJetCounts[iPer][0]->Integral (1, zeeJetCounts[iPer][0]->GetNbinsX(), eta_lo, eta_hi, 1, numphibins);
        countsMC = zeeJetCounts[iPer][1]->Integral () - zeeJetCounts[iPer][1]->Integral (1, zeeJetCounts[iPer][1]->GetNbinsX(), eta_lo, eta_hi, 1, numphibins);
      }
      else {
        countsData = zeeJetCounts[iPer][0]->Integral (1, zeeJetCounts[iPer][0]->GetNbinsX(), eta_lo, eta_hi, 1, numphibins);
        countsMC = zeeJetCounts[iPer][1]->Integral (1, zeeJetCounts[iPer][1]->GetNbinsX(), eta_lo, eta_hi, 1, numphibins);
      }
      ...
      if (eta_lo != 1 || eta_hi != numetabins)
        if (!(exclusive && iEta == numetabins))
          myText (0.155, 0.135, kBlack, Form ("Z (ee) + Jet, %g < #eta_{det}^{Jet} < %g", etabins[eta_lo-1], etabins[eta_hi]), 0.032/uPadY);
        else
          myText (0.155, 0.135, kBlack, Form ("Z (ee) + Jet, %g < #left|#eta_{det}^{Jet}#right|", etabins[eta_hi]), 0.032/uPadY);
      else
        myText (0.155, 0.14, kBlack, "Z (ee) + Jet", 0.032/uPadY);
      myText (0.155, 0.08, kBlack, period, 0.032/uPadY);

      bottomPad->cd ();
      bottomPad->SetLogx ();

      vJetHist_rat = GetDataOverMC (Form ("zeeJetPtDataMCRatio_iEta%i", iEta), proj, proj_mc, numpbins, pbins, true, "x");
      vJetHist_rat_lo = GetDataOverMC (Form ("zeeJetPtDataMCRatio_lo_iEta%i", iEta), proj_lo, proj_mc, numpbins, pbins, true, "x");
      vJetHist_rat_hi = GetDataOverMC (Form ("zeeJetPtDataMCRatio_hi_iEta%i", iEta), proj_hi, proj_mc, numpbins, pbins, true, "x");
    ...
    for (short iP = 0; iP <= numpbins; iP++) {
      ...
      proj = Project2D ("", zeeJetHists_pt_eta[iPer][0][1], "y", "z", p_lo, p_hi);
      proj->RebinY (rebinFactor);
      ...
      // Now calculate systematics by taking the TProfile of the pt+err and pt-err samples, then set as the errors to the TGraphAsymmErrors object
      proj_lo = Project2D ("", zeeJetHists_pt_eta[iPer][0][0], "y", "z", p_lo, p_hi);
      proj_lo->RebinY (rebinFactor);
      vJetHist_lo = GetProfileX ("vJetHist_lo", proj_lo, numetabins, etabins, true);

      proj_hi = Project2D ("", zeeJetHists_pt_eta[iPer][0][2], "y", "z", p_lo, p_hi);
      proj_hi->RebinY (rebinFactor);
      ...
      proj_mc = Project2D ("", zeeJetHists_pt_eta[iPer][1][1], "y", "z", p_lo, p_hi);
      proj_mc->RebinY (rebinFactor);
      ...
      const int countsData = zeeJetCounts[iPer][0]->Integral (p_lo, p_hi, 1, zeeJetCounts[iPer][0]->GetNbinsY(), 1, numphibins);
      const int countsMC = zeeJetCounts[iPer][1]->Integral (p_lo, p_hi, 1, zeeJetCounts[iPer][1]->GetNbinsY(), 1, numphibins);

      myMarkerText (0.175, 0.88, dataColor, dataStyle, Form ("2016 Data (%i events)", countsData), 1.25, 0.032/uPadY);
      myMarkerText (0.175, 0.82, mcColor, mcStyle, Form ("Pythia8 (%i events)", countsMC), 1.25, 0.032/uPadY);
      myText (0.155, 0.2, kBlack, "#bf{#it{ATLAS}} Internal", 0.032/uPadY);
      if (p_lo != 1 || p_hi != numpbins)
        myText (0.155, 0.135, kBlack, Form ("Z (ee) + Jet, %g < #it{p}_{T}^{Z} < %g", pbins[p_lo-1], pbins[p_hi]), 0.032/uPadY);
      else
        myText (0.155, 0.14, kBlack, "Z (ee) + Jet", 0.032/uPadY);
      myText (0.155, 0.08, kBlack, period, 0.032/uPadY);
      ...
      vJetHist_rat = GetDataOverMC (Form ("zeeJetPtDataMCRatio_iP%i", iP), proj, proj_mc, numetabins, etabins, true, "x");
      vJetHist_rat_lo = GetDataOverMC (Form ("zeeJetPtDataMCRatio_lo_iP%i", iP), proj_lo, proj_mc, numetabins, etabins, true, "x");
      vJetHist_rat_hi = GetDataOverMC (Form ("zeeJetPtDataMCRatio_hi_iP%i", iP), proj_hi, proj_mc, numetabins, etabins, true, "x");
    ...
    for (short iEta = 0; iEta <= numetabins; iEta++) { // loop over bins in eta
      ...
      for (short iP = 0; iP <= numpbins; iP++) {
        ...
        int countsData = 0, countsMC = 0;
        if (exclusive && iEta == numetabins) {
          countsData = zeeJetCounts[iPer][0]->Integral () - zeeJetCounts[iPer][0]->Integral (1, numpbins, eta_lo, eta_hi, 1, numphibins);
          countsMC = zeeJetCounts[iPer][1]->Integral () - zeeJetCounts[iPer][1]->Integral (1, numpbins, eta_lo, eta_hi, 1, numphibins);
        }
        else {
          countsData = zeeJetCounts[iPer][0]->Integral (1, numpbins, eta_lo, eta_hi, 1, numphibins);
          countsMC = zeeJetCounts[iPer][1]->Integral (1, numpbins, eta_lo, eta_hi, 1, numphibins);
        }
        ...
        myText (0.155, 0.43, dataColor, Form ("Z (ee) + Jet, %s", period), 0.04);
        ...
        //if (iEta == 0 && iP == 8) {
        //  myText (0.155, 0.43, dataColor, Form ("Z (ee) + Jet, %s", period), 0.04/gPad->GetHNDC ());
