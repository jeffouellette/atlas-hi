                   ...
                   double luminosity) // Integrated luminosity for this run. Presumed constant over the run period.
{
    ...
    for (int etabin = 0; etabin < numetabins; etabin++) {
        xHistArr[etabin] = new TH1D(Form("%ieta%i", thisRunNumber, etabin), Form("%1.1f < #eta < %1.1f;#it{x}_{p};d^{2}#sigma/d#it{x}_{p}d#eta #left[nb#right]", etabins[etabin], etabins[etabin+1]), numxbins, xbins);
        xHistArr[etabin]->Sumw2();
    }
    for (int etabin = numetabins; etabin < 2*numetabins; etabin++) {
        xHistArr[etabin] = new TH1D(Form("%ieta%i", thisRunNumber, etabin), Form("%1.1f < #eta < %1.1f;#it{x}_{a};d^{2}#sigma/d#it{x}_{a}d#eta #left[nb#right]", etabins[etabin%numetabins], etabins[(etabin%numetabins)+1]), numxbins, xbins);
        xHistArr[etabin]->Sumw2();
    }

    for (int qbin = 0; qbin < numqbins; qbin++) {
        xqHistArr[qbin] = new TH1D(Form("%iq%i", thisRunNumber, qbin), Form("%g < #it{Q} < %g;#it{x}_{p};d^{2}#sigma/d#it{x}_{p}d#it{Q} #left[nb GeV^{-1}#right]", qbins[qbin], qbins[qbin+1]), numxbins, xbins);
        xqHistArr[qbin]->Sumw2();
    }
    for (int qbin = numqbins; qbin < 2*numqbins; qbin++) {
        xqHistArr[qbin] = new TH1D(Form("%iq%i", thisRunNumber, qbin), Form("%g < #it{Q} < %g;#it{x}_{a};d^{2}#sigma/d#it{x}_{a}d#it{Q} #left[nb GeV^{-1}#right]", qbins[qbin%numqbins], qbins[(qbin%numqbins)+1]), numxbins, xbins);
        xqHistArr[qbin]->Sumw2();
    }

    for (int etabin = 0; etabin < numetabins; etabin++) {
        mHistArr[etabin] = new TH1D(Form("mjj_%ieta%i", thisRunNumber, etabin), Form("%1.1f < #eta < %1.1f;#it{m}_{JJ} #left[GeV#right];d^{2}#sigma/d#it{m}_{JJ}d#eta #left[nb GeV^{-1}#right]", etabins[etabin], etabins[etabin+1]), nummbins, mbins);
        mHistArr[etabin]->Sumw2();
    }

    TH2D* qxcorr = new TH2D(Form("xqcorr_run%i", thisRunNumber), ";#it{x}_{a};#it{#bar{Q}}^{2} #left[GeV^{2}#right];d^{2}#sigma/d{x}_{a}d#it{#bar{Q}}^{2} #left[nb GeV^{-2}#right]", numq2xbins, q2xbins, numq2bins, q2bins);
    ...
    double leadingjpt, leadingjeta, subleadingjpt, subleadingjeta, leadingjphi, subleadingjphi, leadingje, subleadingje, subsubleadingjpt;
    ...
    for (long long entry = 0; entry < numentries; entry++) {
        ...
        if ((nvert == 0) || (nvert > 0 && vert_type[0] != 1) || njet < 2) continue; // Basic event selection: require a primary vertex and there to be 2 jets
        ...
        if (njet > 2) {
            ...
            for (int j = 1; j < njet; j++) {
                if (j_pt[j] > j_pt[leadingj]) {
                    subsubleadingj = subleadingj;
                    subleadingj = leadingj;
                    leadingj = j;
                }
                else if (j_pt[j] > j_pt[subleadingj]) {
                    subsubleadingj = subleadingj;
                    subleadingj = j;
                }
                else if (j_pt[j] > j_pt[subsubleadingj]) {
                    subsubleadingj = j;
                }
            }
        } else if (j_pt[1] > j_pt[0]) {
            leadingj = 1;
            subleadingj = 0;
        }
        ...
        if (leadingjpt < dijetMinimumPt || subleadingjpt < dijetMinimumPt || subsubleadingjpt > dijetMinimumPt) continue;
        ...
        if (subleadingjpt/leadingjpt < dijet_pt_ratio_cutoff) continue; // cut on dijet events
        ...
        takeEvent = bestTrigger->m_trig_bool && bestTrigger->m_trig_prescale > 0 && leadingjpt >= bestTrigger->min_pt && lumi != 0 && eff != 0.;
        if (!takeEvent) continue; // make sure the trigger fired and was not prescaled, and that we're not going to be dividing by 0
