...
int main (int argc, char *argv[]) {
  ...
  if (argc != 4) {
    ...
    std::cout << " usage: ./mbgen MINPT NEVT FILENAMEOUT" << std::endl;
  ...
  for (int iEvent = 0; iEvent < NEVT; iEvent++) {
    ...
    for (int i = 0; i < pythia.event.size (); i++) {
      ...
      if (pythia.event[i].isHadron () && pythia.event[i].pT () >= 1 && pythia.event[i].isCharged () && fabs (pythia.event[i].pT ()) < 2.5) {
