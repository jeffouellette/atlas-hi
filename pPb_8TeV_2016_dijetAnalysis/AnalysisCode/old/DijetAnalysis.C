                   ...
                   const bool isPeriodA) // period A flag
{
  ...
  const bool periodA = isPeriodA(dataSet, isMC);
  ...
  std::set<TString> activeBranches = {"njet", "jet_pt", "jet_eta", "jet_phi", "jet_e", "truth_njet", "truth_jet_pt", "truth_jet_eta", "truth_jet_phi", "truth_jet_e", "nvert", "vert_type", "fcalA_et", "fcalC_et", "ntrk", "trk_quality4", "trk_d0", "trk_z0", "trk_theta", "trk_charge", "trk_pt", "trk_eta", "trk_phi", "vert_x", "vert_y", "vert_z"};
  ...
  if (!isMC) {
   ...
   TH2D* subJetEtaPhiHist = (TH2D*)etaPhiFile->Get("subJetEtaPhiHist");
  ...
  outFileName = Form("%sdataSet_%i.root", outFileName.c_str(), dataSet);
  ...
  TH2D* qxcorr = new TH2D(Form("xqcorr_dSet%i", dataSet), ";#it{x}_{a};#it{#bar{Q}}^{2} #left[GeV^{2}#right];d^{2}N/L_{int}d{x}_{a}d#it{#bar{Q}}^{2} #left[nb GeV^{-2}#right]", numq2xbins, q2xbins, numq2bins, q2bins);
  qxcorr->SetDirectory(outputFile);
  TH2D* xaxpcorr = new TH2D(Form("xaxpcorr_dSet%i", dataSet), ";#it{x}_{a};#it{x}_{p};d^{2}N/L_{int}d#it{x}_{p}d#it{x}_{a}", numxbins, xbins, numxbins, xbins);
  xaxpcorr->SetDirectory(outputFile);
  TH2D* fcalhist = new TH2D(Form("fcalhist_dSet%i", dataSet), ";#it{x}_{p};FCAL energy #left[GeV#right];", numxbins, xbins, numfcalbins, fcalbins);
  ...
  if (!isMC) {
   for (Trigger* trig : (*triggerSubvector)) {
    tree->SetBranchAddress(Form("%s", trig->name.c_str()), &(trig->trigBool));
    if (containsPeriodLetter) tree->SetBranchAddress(Form("%s_prescale_A", trig->name.c_str()), &(trig->trigPrescale));
    else tree->SetBranchAddress(Form("%s_prescale", trig->name.c_str()), &(trig->trigPrescale));
   }
  }
  ...
  TH1I* eventSelectionHist = new TH1I(Form("eventSelectionHist_dSet%i", dataSet), ";Event selection combination;\"Dijet\" events", 6, -0.5, 5.5);
  ...
  for (long long entry = 0; entry < numentries; entry++) {
   ...
   if (njet >= 3) subsubleadingjpt = t->jet_pt->at(subsubleadingj);
   ...
   if (InDisabledHEC (leadingjeta, leadingjphi)) continue;
   if (InDisabledHEC (subleadingjeta, subleadingjphi)) continue; // select outside disabled HEC region
   ...
   if (!isMC) {
    ...
    takeEvent = bestTrigger->trigBool && bestTrigger->trigPrescale > 0 && bestTrigger->min_pt <= leadingjpt;
