                   ...
                   const bool isPeriodA) // period A flag
{
  ...
  for (long long entry = 0; entry < numentries; entry++) {
   ...
   if ((t->nvert == 0) || (t->nvert > 0 && t->vert_type->at(0) != 1) || t->njet < 2) continue;
   ...
   if (t->njet >= 3) subsubleadingjpt = t->jet_pt->at(subsubleadingj);
   ...
   if (leadingjpt > 1200) cout << Form("High pt (%.0f GeV) jet detected in run %i, event %i!", leadingjpt, dataSet, t->eventNumber) << endl;
