  ...
  for (int iTrig = 0; iTrig < photonTrigN; iTrig++) {
   ...
   for (short iRunNumber = 0; iRunNumber < numRunNumbers; iRunNumber++) {
    ...
    if (photonCounts[iTrig]->GetBinContent (iRunNumber+1) > 0)
     t->DrawText (iRunNumber, 0.1*photonCounts[iTrig]->GetMaximum (), to_string(runNumbers[iRunNumber]).c_str ());
   }
   ...
   for (int iEta = 0; iEta < 2; iEta++) {
    for (short iEtcone = 0; iEtcone < 3; iEtcone++) {
     ...
     for (short iAngle = 0; iAngle < 3; iAngle++) {
      ...
      thisHist->GetXaxis ()->SetTitleSize (0.032/gPad->GetHNDC ());
      thisHist->GetYaxis ()->SetTitleSize (0.032/gPad->GetHNDC ());
      thisHist->GetXaxis ()->SetLabelSize (0.032/gPad->GetHNDC ());
      thisHist->GetYaxis ()->SetLabelSize (0.032/gPad->GetHNDC ());
      ...
      if (iAngle == 0)
       myText (0.55, 0.88, kBlack, etconeStrs[iEtcone], 0.032/gPad->GetHNDC ());
      if (iEtcone == 1 && iAngle == 0) {
       myText (0.55, 0.76, kBlack, (TString(iEta==0?"Barrel":"Endcaps") + ", tight photons").Data (), 0.032/gPad->GetHNDC ());
       myText (0.55, 0.64, kBlack, "#it{p}_{T}^{#gamma} > 20 GeV", 0.032/gPad->GetHNDC ());
      }
      else if (iEtcone == 2 && iAngle == 0) {
       myText (0.55, 0.76, kBlack, photonTrigNames[iTrig].c_str (), 0.032/gPad->GetHNDC ());
      }
      if (iEtcone == 0)
       myText (0.55, 0.76-0.12*iAngle, colors[iAngle], evtPlaneStrs[iAngle], 0.032/gPad->GetHNDC ());
     }
    }
