...
double** kinematicLumiVec = NULL; // total exposed luminosity in a kinematic bin. Units: ub^-1
Trigger*** kinematicTriggerVec = NULL; // best trigger to use in a kinematic bin.
 ...
 * Returns the momentum transfer ("hardness") Q for the event.
 */
double get_q(double jpt1, double jpt2) {
  return 0.5*(jpt1 + jpt2);
}
...
vector<TF1*>* GetTriggerEfficiencyFunctions() {
  TFile* thisFile = new TFile((effPath+"allEfficiencyHistograms.root").Data(), "READ");
  vector<TF1*>* triggerEfficiencyFunctions = new vector<TF1*>(numtrigs);
  for (Trigger* trig : triggerVec) {
   int index = trig->index; 
   if (trig->referenceTrigger == trig) {
    (*triggerEfficiencyFunctions)[index] = new TF1(Form("%s_%s", trig->name.c_str(), fittedFunctionType.Data()), "1", pbins[0], pbins[numpbins]);
    continue;
   }
   TGraphAsymmErrors* thisGraph = (TGraphAsymmErrors*)thisFile->Get(Form("%s_t_graph", trig->name.c_str()));
   (*triggerEfficiencyFunctions)[index] = (TF1*)(thisGraph->GetFunction(Form("%s_%s", trig->name.c_str(), fittedFunctionType.Data())));
  }
...
double** GetTriggerLuminosities() {
  ...
  ifstream luminositiesTxt((homePath + "pPbJetTriggerLuminosities.txt").Data());
  ...
  while(luminositiesTxt) {
    ...
    //cout << thisTriggerName << " endl" << endl;
    for (Trigger* trig : triggerVec) {
      //cout << "trig->name = " << trig->name << " endl" << endl;
  ...
  for (short etabin = 0; etabin < numetabins; etabin++) { 
    ...
    for (short pbin = 0; pbin < numpbins; pbin++) {
      for (Trigger* trig : (*triggerSubvector)) {
        if (!(trig->lowerEta <= etabins[etabin] && etabins[etabin+1] <= trig->upperEta && trig->minPt <= pbins[pbin])) continue;
        thisTrigLumi = totalLumiVec[trig->index][rnIndex];
        if ((bestTrigger == NULL) || (bestTrigLumi < thisTrigLumi) || (bestTrigLumi == thisTrigLumi && bestTrigger->minPt < trig->minPt)) {
...
//  if (!isMC)
//   return TString(dataSet);
//  if (dataSet == 6) { //then it is a JZ2 validation sample
//   if (pA)
//    return "jetjet.valid.JZ2R04.pPb";
//   else
//    return "jetjet.valid.JZ2R04.Pbp";
//  }
 ...
 */
void Initialize (const int dataSetId, const bool isMC, const bool initTriggerMaps) {
  ...
  SetupDirectories(Form("v%i/", useDataVersion), "DijetAnalysis/");
  ...
  if (initTriggerMaps) {
    ...
    kinematicLumiVec = new double*[numpbins];
    kinematicTriggerVec = new Trigger**[numpbins];
    for (short pbin = 0; pbin < numpbins; pbin++) {
     kinematicLumiVec[pbin] = new double[numetabins]; // initialize "array of arrays"
     kinematicTriggerVec[pbin] = new Trigger*[numetabins];
     for (short etabin = 0; etabin < numetabins; etabin++) {
