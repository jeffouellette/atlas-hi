...
void DijetAnalysisHist() {
  ...
  vector<int>* dataSamples = new vector<int> (0);
  for (short i = 0; i < sizeof (full_run_list)/sizeof (full_run_list[0]); i++) dataSamples->push_back (full_run_list[i]);
          ...
          while ((sysfile = (TSystemFile*)next())) {
            ...
            if (!sysfile->IsDirectory() && fname.EndsWith(".root")) {
              if (fname.Contains(to_string(dataSample))) {
                cout << "Reading in " << xPath + periodSubDir + fname << endl;
...
//  myText (0.43, 0.84, kBlack, Form("%i dijet events", numGoodEvents));
  ...
  qxcorr->GetXaxis()->SetTitleOffset(1.2);
  qxcorr->GetYaxis()->SetTitleOffset(1);
  qxcorr->GetYaxis ()->SetTitle ("#it{Q}^{2} #left[GeV^{2}#right]");
  qxcorr->GetZaxis ()->SetTitle ("Relative Yield");
  qxcorr->Draw("colz");
...
//  xaxpcorr->GetZaxis()->SetTickLength(0.01);
  ...
  //myText (0.17, 0.89, kBlack, "#bf{#it{ATLAS}} Internal", 0.051);
  myText (0.17, 0.89, kBlack, "#sqrt{s} = 8.16 TeV #it{p}+Pb, 161.4 nb^{-1}", 0.051);
  //myText (0.17, 0.77, kBlack, Form("%i dijet events", numGoodEvents), 0.051);
